...
describe("Configuration Drift Detection", () => {
	...
	describe("Quality Profile Drift", () => {
		test("PrepArr recreates deleted quality profile in Sonarr", async () => {
			...
			await waitForCondition(
				async () => {
					const result = await callServarrApi<QualityProfile[]>(
						"sonarr",
						"/api/v3/qualityprofile"
					);
			...
			const finalResult = await callServarrApi<QualityProfile[]>(
				"sonarr",
				"/api/v3/qualityprofile"
			);
	...
	describe("Root Folder Drift", () => {
		test("PrepArr recreates deleted root folder in Sonarr", async () => {
			...
			const initialResult = await callServarrApi<RootFolder[]>(
				"sonarr",
				"/api/v3/rootfolder"
			);
			expect(initialResult.ok).toBe(true);
			...
			const deleteResult = await callServarrApi(
				"sonarr",
				`/api/v3/rootfolder/${tvFolder!.id}`,
				{ method: "DELETE" }
			);
			expect(deleteResult.ok).toBe(true);

			// Verify it's deleted
			const afterDeleteResult = await callServarrApi<RootFolder[]>(
				"sonarr",
				"/api/v3/rootfolder"
			);
			...
			await waitForCondition(
				async () => {
					const result = await callServarrApi<RootFolder[]>(
						"sonarr",
						"/api/v3/rootfolder"
					);
			...
			const finalResult = await callServarrApi<RootFolder[]>(
				"sonarr",
				"/api/v3/rootfolder"
			);
	...
	describe("Download Client Drift", () => {
		test("PrepArr restores disabled download client in Sonarr", async () => {
			...
			await waitForCondition(
				async () => {
					const result = await callServarrApi<DownloadClient[]>(
						"sonarr",
						"/api/v3/downloadclient"
					);
			...
			const finalResult = await callServarrApi<DownloadClient[]>(
				"sonarr",
				"/api/v3/downloadclient"
			);
