...
describe("Initial Setup Flow", () => {
	...
	describe("Quality Profiles", () => {
		test("Sonarr has HD - 1080p quality profile", async () => {
			const result = await callServarrApi<Array<{ name: string }>>("sonarr", "/api/v3/qualityprofile");
		...
		test("Radarr has HD - 1080p quality profile", async () => {
			const result = await callServarrApi<Array<{ name: string }>>("radarr", "/api/v3/qualityprofile");
